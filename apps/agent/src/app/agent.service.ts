...
export class AgentService {
  ...
  private systemPrompt(): string {
    ...
    return [
      ...
      '  start with discover_business_flows. For failures use error_breakdown or find_anomalies,',
      '  then get_trace_flow on a failing trace. For "what just happened" use get_recent_activity.',
      '- For log questions (specific messages, a service\'s errors, metadata like an orderId)',
      '  use search_logs; inspect one trace\'s business data with get_span_metadata; deep-dive',
      '  one service with analyze_service; explain "worked vs failed" with compare_traces.',
      '- For "what changed since yesterday / the deploy?" use compare_flow_windows — it reports',
      '  flows that appeared, disappeared or changed error rate, latency or traffic.',
      '- When you reach a durable conclusion (a flow change, an error pattern, an anomaly),',
      '  persist it with save_insight including the evidencing trace ids; check list_insights',
      '  before re-analyzing a question that may already be answered.',
      '- For "is anything wrong right now?" check list_alert_events first. When the user asks',
      '  to be told/alerted about something in the future, set up a watch with',
      '  create_alert_rule (e.g. flow-missing on a flow signature, error-rate on a threshold).',
